...
class ShootingGame implements Game {
  ...
  public void drawTitle() {
    ...
    textSize(60);
    text("シューティングゲーム", width/2, height/3);
    textSize(30);
    text("クリックで弾を撃ち、敵のバーに5回当てると勝ち！", width/2, height/2);
    text("ENTERでスタート", width/2, height*0.75f);
  }
  ...
  public void drawBack() {
    ...
    textAlign(CENTER, CENTER);
    textSize(40);
    text("連勝おめでとう！休憩を忘れずに。", width/2, height/2);
    textSize(20);
    text("TABキーでタイトルへ戻る", width/2, height/2 + 50);
  }
...
// エアホッケーゲームのクラス
  ...
  public void initialize() {
  rectMode(CORNER); // ← ここでリセット！
  textAlign(LEFT, BASELINE); // 必要に応じて
  font = createFont("ＭＳ Ｐゴシック", 50);
  textFont(font);
  resetBall();
}
  ...
  public void drawTitle() {
    ...
    fill(255);
    textSize(40);
    text("勝利条件：相手より先に target score へ到達せよ", width / 16, height / 2 - 150);
    text("ボールが紫色のラインを越えたら得点だよ", width / 16, height / 2 - 50);
...
class GameSelector {
  ...
  void drawGame() {
    if (inMenu) {
      ...
      textSize(30);
      fill(200);
      text("Use UP/DOWN keys to select, ENTER to start", width/2, height - 40);
